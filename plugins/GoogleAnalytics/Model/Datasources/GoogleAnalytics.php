...
class GoogleAnalytics extends DataSource
{
    var $Http = null;
    var $token = '';
...
// 
    ...
    function accounts()
    {
        ...
        foreach ($entries as $entry)
        {
            ...
            $properties = Set::combine($entry, 'dxp:property.{n}.@name', 'dxp:property.{n}.@value');
            
            $accountId = $properties['ga:accountId'];
            $accountName = $properties['ga:accountName'];
            $profileId = $properties['ga:profileId'];
            $webPropertyId = $properties['ga:webPropertyId'];
...
//             )));
    ...
    function __validateQueryData($queryData)
    {
        ...
        {
            trigger_error(
                __('too many dimensions, the maximum allowed is 7'),
                E_USER_ERROR);
            return null;
        }
        if (!empty($conditions['metrics']) &&
        ...
        {
            trigger_error(
                __('too many metrics, the maximum allowed is 10'),
                E_USER_ERROR);
            return null;
        }
