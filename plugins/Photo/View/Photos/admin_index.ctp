...
$ajaxLoaderImage = $this->Html->image('cms/ajax-loader.gif', array('alt' => __('Ładowanie...')));
    ...
    var initQtip = function(element) {
        $(element).qtip({
            content: {
                ...
                title: {
                    text: 'Ustaw tytuł', // Give the tooltip a title using each elements text
                    button: true
                }
            },
            position: {
...
<div class="photos index">
    <h2><?php echo __d('cms', 'Galeria zdjęć: '); ?><div id="photoHelp">?</div></h2>
...
<div class="actions">
    ...
    <ul>
        <?php echo $this->Permissions->link(__d('cms', 'Edit '.$model), array('plugin' => !empty($plugin)?strtolower($plugin):false,'controller' => Inflector::tableize($model), 'action' => 'edit', $remote_id), array('outter' => '<li>%s</li>')); ?>
        <?php echo $this->Permissions->link(__d('cms', 'List '.Inflector::pluralize($model)), array('plugin' => !empty($plugin)?strtolower($plugin):false, 'controller' => Inflector::tableize($model), 'action' => 'index', $remote_id), array('outter' => '<li>%s</li>')); ?>
    </ul>
</div>
