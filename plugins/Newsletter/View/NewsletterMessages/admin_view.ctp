...
::link wypisania z newslettera::';
        ...
        <?php if ($newsletterMessage['NewsletterMessage']['recipients'] == 0) { ?>
            ...
            <?php echo $this->Form->submit(__d('cms', 'Wyślij %s', 'wiadomość newslettera')); ?>
    ...
    <?php if ($newsletterMessage['NewsletterMessage']['recipients'] == 0) { ?>
        ...
        <div class="actions">
            <ul>
                <li><?php echo $this->Html->link(__d('cms', 'Edytuj %s', 'wiadomość newslettera'), array('action' => 'edit', $newsletterMessage['NewsletterMessage']['id'])); ?> </li>
                <li><?php echo $this->Html->link(__d('cms', 'Usuń %s', 'wiadomość newslettera'), array('action' => 'delete', $newsletterMessage['NewsletterMessage']['id']), null, __d('cms', 'Jesteś pewien, że chcesz usunąć wiadomość o temacie: "%s"?', $newsletterMessage['NewsletterMessage']['title'])); ?> </li>
                <li><?php echo $this->Html->link(__d('cms', 'Lista %s', 'wiadomości newslettera'), array('action' => 'index')); ?> </li>
            </ul>
        </div>
    <?php } else { ?>
