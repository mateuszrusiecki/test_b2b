...
class BanersController extends AppController {
    ...
    public function rd($id = null) {
        if (!$id) {
            throw new NotFoundException(__d('public', 'Nie odnaleziono'));
        }
     ...
     */
    public function get_baner($group = null) {
        $this->layout = 'ajax';
        ...
        $this->Baner->recursive = -1;
        $baner = $this->Baner->find('first', $params);
    ...
    public function admin_edit($id = null) {
        if (!$id && empty($this->request->data)) {
            $this->Session->setFlash(__('Invalid baner'));
            $this->redirect(array('action' => 'index'));
        }
        if (!empty($this->request->data)) {
    ...
    public function admin_delete($id = null) {
        ...
        if (!$this->Baner->exists()) {
            throw new NotFoundException(__('Invalid id for baner'));
        }
        if ($this->Baner->delete($id)) {
            $this->Session->setFlash(__('Baner deleted'));
            $this->redirect(array('action' => 'index'));
        }
        $this->Session->setFlash(__('Baner was not deleted'));
        $this->redirect(array('action' => 'index'));
    }
