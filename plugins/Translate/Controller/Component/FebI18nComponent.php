...
class FebI18nComponent extends Object
{
    ...
    {
        $userLang = $this->controller->Session->read('Auth.User.lang');
        $sessionLang = $this->controller->Session->read('Config.language');
        $sessionLang = empty($sessionLang) ? DEFAULT_LANGUAGE : $sessionLang;
        $lang = empty($userLang) ? $sessionLang : $userLang;
        //$lang = empty($this->controller->params['lang']) ? $sessionLang : $this->controller->params['lang'];
     ...
     * @access protected
     */
    protected function setLanguage2_old()
    {
        ...
        } else
        {
            ...
            if ($lang == DEFAULT_LANGUAGE AND ! empty($this->controller->request->here) AND $this->controller->request->here == '/')
            {
                ...
                {
                    $this->controller->redirect(array('controller' => 'fronts', 'action' => 'front', 'lang' => $lang), 303);
                }
            } else
            {
                $this->controller->Session->write('Config.language', $lang);
...
// 
//         if($error){
//             $setFlashSession=__('Nie udało się zapisać');
//             foreach($validateErrors as $input => $message){
//                 $setFlashSession.=' ,'.$message;
//                 if(strpos($input,'_lang_')){
//                     $input=explode('_lang_',$input);
//                     $setFlashSession.= __(' w języku ').$input[0];
//                 }
//             }
//             $setFlashSession.= ' '.__('sprawdź formularz i spróbuj ponownie');
