    ...
    function delete($id= null, $all = 0, $model = null){
        ...
        if ($all or (!$all and $ile1 == $ile2)) {
            if($this->controller->$model->delete($id)){
                $this->controller->Session->setFlash(__('Pozycja została usunieta'));
                return true;
            }else{
                $this->controller->Session->setFlash(__('Pozycja nie została usunięta'));
                return false;
            }         
        } else {
            if($this->controller->$model->query("DELETE FROM `i18n` WHERE `model` = '{$model}' AND locale = '{$lang}' AND foreign_key = {$id}")){
                $this->controller->Session->setFlash(__('Język został usunięty'));
                return true;
            }else{
                $this->controller->Session->setFlash(__('Język nie został usunięty'));
                return false;
            }
        }
    }
