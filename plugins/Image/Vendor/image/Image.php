...
//                debug('Destination dir is not writable.');
        ...
        if (!empty($imageOptions['crop'])) {
            list($imageOptions['x'], $imageOptions['y'], $imageOptions['w'], $imageOptions['h']) = $this->cropCords($imageOptions, $srcData);
        }
        if ((isset($imageOptions['x']) && !is_null($imageOptions['x'])) && (isset($imageOptions['y']) && !is_null($imageOptions['y'])) && (isset($imageOptions['w']) && !is_null($imageOptions['w'])) && (isset($imageOptions['h']) && !is_null($imageOptions['h']))) {
            $this->resize($imageOptions['width'], $imageOptions['height'], $imageOptions['x'], $imageOptions['y'], $imageOptions['w'], $imageOptions['h'], ASIDO_RESIZE_STRETCH);
        } else {
    ...
    function resize($width, $height, $x = null, $y = null, $w = null, $h = null, $mode = ASIDO_RESIZE_PROPORTIONAL) {
        return Asido::resize($this->res, $width, $height, $x, $y, $w, $h, $mode);
    }
