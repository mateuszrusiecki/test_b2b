...
class UploadBehavior extends ModelBehavior {
     ...
     * @access public
     */
    function setup(Model $model=null, $settings = array()) {
...
//debug($variableValue['type']);exit;        	
                ...
                if(!$error){
                    if($maxFileSize !== null){
                        ...
                        if(isSet($fieldValue['size']) AND $fieldValue['size'] > $maxFileSize){
                            $error = __("Tym formularzem nie można wysłać pliku większego niż ".(round($maxFileSize/1024/1024*100)/100)." MB. Zmniejsz plik lub wybierz inny.");
                        }
                    }
                    ...
                    if(!$error and $maxImageSize !== null){
                        ...
                        if(!empty($size) and ($size[0] > $maxImageSize['width'] || $size[1] > $maxImageSize['height'])){
                            $error = __("Tym formularzem nie można wysłać obrazka większego niż {$maxImageSize['width']}px x {$maxImageSize['height']}px");
                        }
                    }
    ...
    function _deleteFile($name, $modelName){
        ...
        foreach($readed as $dir) {
            if(!is_dir($destDir.DS.$dir) OR $dir == '.' OR $dir == '..'){
                continue;
            }
    ...
    function areUploadErrors($array, $required = false, $allowEmpty = true, $maxFileSize = null){
        ...
        if($required && (empty($array['name']) || empty($array['tmp_name'])) && (!isSet($array['error']) || $array['error'] == UPLOAD_ERR_OK)){
            return __("Nie wysłano pliku.");
        } elseif(isSet($array['size']) && $array['size'] == 0 && !$allowEmpty) {
            return __("Wysyłany plik nie może być pusty");
        } else {
            switch($array['error']){
                //// TODO:
                ...
                case UPLOAD_ERR_INI_SIZE:
                    return __("Maksymalny dopuszczalny rozmiar pliku na tym serwerze to ".ini_get('upload_max_filesize'));
...
//                    debug(MAX_FILE_SIZE); exit; //{$this->settings[$model->alias]['maxFileSize']}
                    return ($maxFileSize)?__("Tym formularzem nie można wysłać pliku większego niż ".(round($maxFileSize/1024/1024*100)/100)." MB. Zmniejsz plik lub wybierz inny."):__("Tym formularzem nie można wysłać pliku tej wielkości");
                ...
                default:
                    return __("Unknown exception while file upload");
            }
        }
    }
