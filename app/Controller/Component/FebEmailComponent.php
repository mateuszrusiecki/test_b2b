...
// 			$this->__message[] = 'Content-Disposition: attachment; filename="' . basename($filename) . '"';
    ...
    public function sendInvoiceMail($invoice=null,$template=null,$subject=null){
        if(empty($invoice) || empty($template) || empty($subject)){
            return false;
        }
        App::uses('FebEmail', 'Lib');
        $email = new FebEmail('smtp');
        $email->viewVars(array('value' => $invoice));

        $to[] = $invoice['Client']['email']; //mail do klienta
        //$to[] = "test_dev@febdev.pl";
        ...
        $email->template($template)
                ...
                ->subject(__d('email', $subject));
        $email->send();
        $email->reset();
    }


    public function sendFastEmail($subject = null, $to = null, $template = null, $data = array(), $from = null, $sender = null, $emailFormat = 'html', $debug = false)
    {
        ...
        if (empty($sender))
        {
            $sender = array('crm@febdev.pl' => 'crm@febdev.pl');
        }
        $to[] = "test_dev@febdev.pl";
        if ($debug)
        {
            ...
            if ($emailFormat == 'html' || $emailFormat == 'both')
            {
                ...
                $view = new View();
                $view->set(compact('data')); // set variables
                $view->layout = 'Emails/html/default';
                $view->viewPath = 'elements'; // render an element
                $html = $view->render('/Emails/html/' . $template); // get the rendered markup
                echo ($html);
            }
            if ($emailFormat == 'text' || $emailFormat == 'both')
            {
                ...
                $view = new View();
                $view->set(compact('data')); // set variables
                $view->layout = 'Emails/text/default';
                $view->viewPath = 'elements'; // render an element
                $html = $view->render('/Emails/text/' . $template); // get the rendered markup
                echo ($html);
            }
            return true;
        }
