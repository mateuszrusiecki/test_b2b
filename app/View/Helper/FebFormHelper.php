...
class FebFormHelper extends AppHelper {
    ...
    function file($fieldName, $options = array()) {
        ...
        if ($val && !is_array($val)) {
            ...
            if (!isSet($delete)) {
                $input .= $this->Form->input($fieldName . '_delete', array('type' => 'checkbox', 'label' => 'usuń plik'));
            } elseif ($delete !== false) {
                $input .= $delete;
            }
        }
    ...
    function multiselect($fieldName, $options = array(), $autocompleteOptions = array()) {
        ...
        if (!empty($this->request->data['_autocomplete'][$model])) { //Przy akcji add
            foreach ($this->request->data['_autocomplete'][$model] as $key => $displayField) {
                $autocompleteOptions['value'] = $displayField;
                $autocompleteOptions['hiddenValue'] = $this->request->data[$model][$model][$key];
                $optionsForHiddenInput['value'] = $this->request->data[$model][$model][$key];
        ...
        } elseif (!empty($this->request->data[$model]) && is_array($this->request->data[$model])) {  //Przy akcji edit
...
//        
    ...
    function autocomplete($fieldName, $options = array(), $addHiddenInput = true) {
        ...
        if (!empty($this->request->data[$model][$fieldAutocompleteName]) && !empty($options['options']) && !is_array($this->request->data[$model][$fieldAutocompleteName])) {
            $options['value'] = $options['options'][$this->request->data[$model][$fieldAutocompleteName]];
        }
     ...
     * @return type 
     */
    public function upload($model, $remote_id, $initParams = array(), $callbacks = array()) {
        ...
        $uploadUrl = $this->Html->url(array('admin' => false,'controller' => 'base_projects', 'action' => 'upload', $model, $remote_id));
        ...
        $initScript = "$(function(){
                ...
                new qq.FileUploader({
                        ...
                        debug: false,
                        sizeLimit: 3300000,   
                        allowedExtensions: ['jpg', 'png', 'gif', 'psd'],   
