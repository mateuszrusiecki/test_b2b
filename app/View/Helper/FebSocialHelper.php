    ...
    function simpleBtns($names) {
        if (is_array($names) && count($names) > 0) {
            ...
            foreach ($names as $key => &$btn) {
                ...
                if ($key === 'wykop') {
                    $sthis .= '<a target="_blank" title="Wykop.pl" href="http://www.wykop.pl/dodaj?url='.Router::url($this->request->params['url']['url'],true).'&amp;title='.$btn['title'].'">'.$this->Html->image('social/wykop.png').'</a>';
                } else {
