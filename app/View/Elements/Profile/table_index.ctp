    ...
    <div class="clearfix row">
        <form class="form ng-pristine ng-valid">
            ...
            <div class="col-sm-4 col-xs-12">
                <a ng-click="search_user_id = !search_user_id" ng-class="{'blue':search_user_id}" class="poitnier btn btn-sm   margin-bottom pull-left default margin-top-10"><?php echo __d('public', 'Kandydaci') ?></a>
            </div>
        </form>
    </div>
                ...
                <tbody>
                    <tr ng-cloak ng-repeat="profile in profiles | filter:search | candidates:search_user_id | orderBy:order:reverse | pag: currentPage : 10">
                        ...
                        <td class='vertical-middle  hidden-xs'>
                            <strong>
                                {{profile.Profile.position|| "brak danych"}}
                            </strong><br />
                            {{profile.Profile.place_of_work|| "brak danych"}}
                        </td>
                        <td class='vertical-middle'>
                            {{profile.Profile.private_phone|| "brak danych"}}<br />
                            ...
                            <div ng-if="!profile.User.email">                                
                                <?php echo __d('public', 'brak danych') ?>
                            </div>
                        </td>
...
<!--                            <div ng-if="profile.User.active == 1">
                        ...
                        <td class='vertical-middle hidden-xs'>
                            <div ng-if="profile.Profile.user_id">
                                <i class="fa fa-ambulance font-green font-large ml" tooltip="<?php echo __d('public', 'Brak danych o badaniach') ?>"></i>
                                <i class="fa fa-graduation-cap font-red-pink font-large ml" tooltip="<?php echo __d('public', 'Brak danych o szkoleniu') ?>"></i>
                                <a href="/hrs/add_contract/{{profile.Profile.id}}"><i class="fa fa-warning  font-red-pink font-large ml" ng-if="!profile.UserContractHistories.employment_start" tooltip="<?php echo __d('public', 'Brak danych o umowie. Kliknij aby dodać') ?>."></i></a>
