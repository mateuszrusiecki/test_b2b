...
CREATE DEFINER=`mbienia`@`%` PROCEDURE `update_project_status`(IN projectId INT UNSIGNED)
BEGIN
...
CREATE DEFINER=`mbienia`@`%` PROCEDURE `update_version_status`(IN versionId INT UNSIGNED)
BEGIN
...
CREATE DEFINER=`mbienia`@`%` PROCEDURE `update_view_status`(IN viewId INT UNSIGNED)
BEGIN
