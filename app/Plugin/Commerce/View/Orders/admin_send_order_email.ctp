...
<fieldset>
    ...
    echo $this->Form->input('Od', array('label' => 'Nadawca', 'readonly' => 'readonly'));
    echo $this->Form->input('Do', array('label' => 'Odbiorca', 'readonly' => 'readonly'));
    echo $this->Form->input('subject', array('label' => 'Temat', 'value' => 'Informacje o realizacji zamówienia nr #' . $order['Order']['hash']));
    echo $this->Form->input('content', array('div' => false, 'label' => false, 'id' => 'email-content', 'type' => 'textarea'));
    echo $this->Form->end(__('Wyślij'));
    echo $this->Html->link(__('Nie Wysyłaj'), array('action' => 'index'), array('class' => 'button'));
    ?>
...
<div id="hiden-tiny-content" style="display: none;">
    ...
    <div class="body">
        <div id="content-for-layout">
            Witaj,<br />Twoje zamówienie zostało zaktualizowane. Szczególy zamówienia możesz sprawdzić poniżej:
            =====================<br />
            ...
            <?php if ($order['Order']['order_status_id'] >= 50 && !empty($order['ShipmentMethod']['track_link'])) { // Jeżeli jest to kurier tnt albo gls ?>
                <?php if ($order['ShipmentMethod']['id'] == 4) { ?>
                    Przesyłkę można śledzić pod adresem: <?php echo $this->Html->link($order['ShipmentMethod']['name'] . ' śledzenie przesyłki', $order['ShipmentMethod']['track_link'] . $order['Order']['track_number'], array('target' => "_blank")); ?><br /><br />        
                <?php } else { ?>
                    Przesyłkę można śledzić pod adresem: <?php echo $this->Html->link($order['ShipmentMethod']['name'] . ' śledzenie przesyłki', $order['ShipmentMethod']['track_link'], array('target' => "_blank")); ?>, wpisując numer przesyłki: <b><?php echo $order['Order']['track_number']; ?></b> <br /><br />
                <?php } ?>
            <?php } ?>
            ...
            <?php if (!empty($order['Payment'])) { ?>
                ...
                <ul>
                    <?php foreach ($order['Payment'] as $k => $payment) { ?>
                        <li><?php echo $payment['payment_gate']; ?> - <?php echo $febNumber->priceFormat($payment['amount']); ?> - <?php echo $statuses[$payment['status']]; ?> <?php echo $payment['payment_date']; ?> </li>
                    <?php } ?>
                </ul>
            <?php } ?>
            ...
            <?php if (isSet($shipmentMethods[$order['Order']['shipment_method_id']])) { ?>    
                <b>Dostawa</b>: <?php echo $shipmentMethods[$order['Order']['shipment_method_id']]; ?> - <?php echo $febNumber->priceFormat($shipmentTemp['price_gross']); ?> <br /> 
            <?php } ?>
            ...
            <b>RAZEM</b> (z kosztami wysyłki): <?php echo $febNumber->priceFormat($summary['final_price_gross']) ?><br />
            <b>Zapłacono:</b> <?php echo $febNumber->priceFormat($paymentTotal); ?><br />
            <b>Pozostało:</b> <?php echo ($summary['final_price_gross'] - $paymentTotal) >= 0 ? $febNumber->priceFormat($summary['final_price_gross'] - $paymentTotal) : 'Nadplata ' . $febNumber->priceFormat($summary['final_price_gross'] - $paymentTotal); ?><br />
            =====================<br />
        </div>
    </div>
</div>
