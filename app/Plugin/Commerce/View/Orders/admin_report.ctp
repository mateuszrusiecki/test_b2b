...
<h2><?php echo __('Raporty sprzedaży');?></h2>

<div class="clearfix">
    <div class="tooltipChart">
        <?php
        $options['count'] = "Ilość zrealizowanych zamówień";
        $options['value'] = "Wartość zrealizowanych zamówień";

        echo $this->Form->input('Typ', array('type' => 'select', 'options' => $options, 'label' => 'Typ wykresu', 'div' => array('id' => 'chartType')));
        echo $this->Form->button('Wykres dzienny', array('id' => 'datePickerDayButton'));
        echo $this->Form->button('Wykres miesięczny', array('id' => 'datePickerMonthButton'));
        echo $this->Form->button('Wykres roczny', array('id' => 'datePickerYearButton'));
        ?>
...
<script type="text/javascript">     
    ...
    function drawChart(dane) {
        ...
        var options = {
            ...
            height: 500,
            title: 'Raporty sprzedaży - ' +dane.config.title,
            vAxis: {
...
<script>
    ...
    function tableCreate(_data) {
        ...
        $('#table_div table').append('<thead><tr><th>Okres rozliczeniowy</th><th>' + _data.config.vAxis + '</th></tr></thead>');
