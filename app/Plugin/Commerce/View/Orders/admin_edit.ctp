...
//    }
    ...
    |
    <?php echo $this->Html->link('Wyślij Wiadomość Email do Klienta', array('action' => 'send_order_email', $order['Order']['id']), array('class' => 'button')); ?>
</div> 
<br /><br />   
<?php echo $this->Html->script('/commerce/js/jquery.jeditable'); ?>
<?php echo $this->Html->script('/commerce/js/commerce'); ?>

<?php echo $this->Html->css('ui-lightness/jquery-ui-1.8.14.custom'); ?>
<?php echo $this->Html->script('jquery-ui-1.8.14.custom.min'); ?>
...
<div id="editOrder" class="orders form">
    ...
    <fieldset id="order-area">        
        <?php //debug($this->Html->);  ?>
        <legend>Zamówienie Numer: <b><?php echo $this->request->data['Order']['hash']; ?></b></legend>
        <b>Data utworzenia zamówienia:</b> <?php echo $this->FebTime->niceShort($this->request->data['Order']['created']); ?><br/>
        <b>Ostatnia modyfikacja:</b> <?php echo $this->FebTime->niceShort($this->request->data['Order']['modified']); ?><br/><br/>
            ...
            <tr>
                <td>
                    <fieldset id="customer-contact">
                        ...
                        <?php echo $this->Form->input('Customer.contact_person', array('readonly' => true, 'label' => __('Imię i Nazwisko'), 'type' => 'text')); ?>
                        ...
                        <div class="clearfix buttonOrange">
                            <?php echo $this->Html->link('Edycja Danych Klienta', array('controller' => 'customers', 'action' => 'edit', $this->request->data['Customer']['id']), array('class'=>'fr')); ?>
                        </div>
                ...
                <td>
                    <fieldset>
                        ...
                        <?php echo $this->Form->input('order_status_id', array('label' => 'Status')); ?>  
                        <?php echo $this->Form->input('track_number', array('label' => 'Nr listu przewozowego')); ?>  
                        <?php echo $this->Form->input('vat', array('label' => 'Wystawiona faktura vat', 'type' => 'checkbox')); ?>  
                    </fieldset>
                </td>
            </tr>
            <tr>
        ...
        <fieldset id="notes">
            ...
            echo $this->FebTinyMce4->input('Note.content', array('name' => 'data[Note][content]', 'id' => 'NoteContent', 'label' => false), 'bbcode');
            ?>
            <div id="notes-area">
                <?php echo $this->element('Note.Notes/notes', array('plugin' => 'commerce', 'notes' => $this->request->data['Note'])); ?>  
            </div>
        </fieldset>
        <?php //debug($diffs) ?>
...
<script type="text/javascript">
    ...
    jQuery('#OrderInvoiceIdentityCountryId, #AddressDefaultCountryId').change(function(){woj()});
    jQuery('#OrderInvoiceIdentityCountryId, #AddressDefaultCountryId').blur(function(){ woj()});
    woj();
        
    function woj(){
        if(jQuery('#OrderInvoiceIdentityCountryId').val() == 'PL'){
