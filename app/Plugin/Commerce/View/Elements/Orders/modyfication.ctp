...
<?php if (!empty($diffs)) { ?>            
    <fieldset>
        <legend>Historia Modyfikacji</legend>
        <?php foreach ($diffs as $changeArray) { ?>

            <?php if ($changeArray['Modification']['model'] == 'Order') { ?>

                <h3>Uaktualnione przez <?php echo $changeArray['User']['name'] ?> <?php echo $this->FebTime->niceShort($changeArray['date']); ?></h3>
                <ul> 
                    ...
                    foreach ($changeArray['diff'] as $klucz => $afterValue) {
                        ...
                        if ($klucz == 'address') {
                            ...
                            <h4>Modyfikacja danych wysyłki:</h4>
                            <?php
                            foreach ($afterValue as $k => $v) {
                                ...
                                <li><b><?php __d('commerce', $k) ?></b> zmieniono z <b><?php echo $changeArray['before']['address'][$k] ?></b> na <b><?php echo $v ?></b></li>
                            <?php } continue; ?>
                        <?php } ?>

                        <?php if ($klucz == 'invoice_identity') { ?>

                            <h4>Modyfikacja danych do faktury:</h4>
                            <?php
                            foreach ($afterValue as $k => $v) {
                            ...
                            <?php }
                            ?>
                                <li><b><?php __d('commerce', $k) ?></b> zmieniono z <b><?php echo $changeArray['before']['invoice_identity'][$k] ?></b> na <b><?php echo $v ?></b></li>
                    <?php } continue; ?>
                    <?php } ?>

                        <li><b><?php __d('commerce', $klucz) ?></b> zmieniono z <b><?php echo $changeArray['before'][$klucz] ?></b> na <b><?php echo $afterValue ?></b></li>
            <?php } ?>
                </ul><br />
                ...
                <h3>Uaktualnione przez <?php echo $changeArray['User']['name'] ?> <?php echo $this->FebTime->niceShort($changeArray['date']); ?></h3>
                <ul> 
                    <h4>Modyfikacja Produktów Zamówienia</h4>
            <?php foreach ($changeArray['diff'] as $key => $value) { ?>

                        <li><b><?php __d('commerce', $key) ?></b> zmieniono z <b><?php echo $changeArray['before'][$key]; ?></b> na <b><?php echo $value; ?></b> dla <b><?php echo $changeArray['before']['name']; ?></b></li>
                <?php } ?>
                </ul><br />
        <?php } ?>
