...
class CustomersController extends CommerceAppController {
    ...
    function beforeFilter() {
        parent::beforeFilter();
        $this->Auth->allow(array('add', 'edit', 'my_orders_in_chart', 'my_orders_active', 'my_orders_status_ended', 'my_settings', 'my_discount', 'my_invoice', 'my_orders', 'order_item', 'order_details'));
    }
    ...
    */
	public function admin_index() {
        ...
        $this->filters = array(
            ...
            'Customer.city' => array('param_name' => 'miejscowosc', 'default' => '', 'form' => array('label' => __('Miejscowość', true))),
        ...
        $this->paginate = $this->Customer->filterParams($this->request->data);

        $myParams['conditions']['Customer.email <>'] = null;
        $this->paginate = $myParams;
    ...
    */
    ...
    function order_item($id = null) {
        ...
        $this->Customer->Order->unbindModel(array('belongsTo' => array('Customer', 'OrderStatus', 'ShipmentMethod'), 'hasOne' => array('Invoice'), 'hasMany' => array('Note', 'Payment')));
...
//Jezeli jest ustawione haslo
            ...
            if ($this->Customer->saveAll($this->request->data)) {
                if (!empty($this->request->data['User']['newpass']) || !empty($this->request->data['User']['oldpass'])) {
                    ...
                    $this->Session->setFlash(__('Hasło zostało poprawnie zmienione. Zaloguj poprawnie.', true));
