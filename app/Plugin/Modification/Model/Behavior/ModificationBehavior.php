    ...
    function beforeValidateLogic($model){
        ...
        if(!empty($row_id)){
            ...
            if(!empty($object[$model->alias])){
                ...
                //zmiana nazwy akcji, jeĹ›li "softdelete":
                ...
                //zmiana nazwy akcji, jeĹ›li "recover":
    ...
    function afterSave (Model $model, $created, $options=array()){
        ...
        if(isSet($this->runtime[$model->alias]['newRecordData'])){
            ...
            if($data['Modification']['foreign_key'] != null){
                ...
                $data['Modification']['content_after'] = $model->findById($data['Modification']['foreign_key']);
                $data['Modification']['content_after'] = $data['Modification']['content_after'][$model->alias];
            
//                 foreach($data['Modification']['content_before'] AS $key => $value){
//                     if(!isSet($data['Modification']['content_after'][$key]) OR $data['Modification']['content_after'][$key] === $value){
// //                        unSet($data['Modification']['content_before'][$key]);
// //                        unSet($data['Modification']['content_after'][$key]);
//                     }
//                 }
