...
// 	}
    ...
    function start($payment_gate = 'platnosci.pl', $id = null){
        ...
        if(empty($payment)){
            ...
            if(empty($payment)){
                $this->Session->setFlash('Próba wykonania płatności nie powiodła się.', 'flash/error');
            } else {
                $this->Session->setFlash('Płatność została już rozpoczęta. Aby powtórzyć płatność przejdź do swojego konta, lub skontatuj się z biurem obsługi klienta.', 'flash/error');
            }
